"""Valuation Debate Agent: wraps ValuationAgent with adversarial debate capability.

Extends the base ValuationAgent to participate in round-robin debates by:
...
You have already completed your initial analysis. Now you must debate with the Search Agent and Sentiment Agent.

Read the prior round positions. Identify the WEAKEST opposing argument — one that:
- Contradicts your findings on market size, comparables, or return potential
- Misunderstands stage-appropriate metrics or valuation benchmarks
- Overstates or understates investment attractiveness
...
Be specific. Cite the agent and argument you're challenging. Use valuation and comparables evidence from your Phase 1 analysis.
"""
...
Read the prior round positions. Identify the WEAKEST opposing argument — one that:
- Downplays market size risks or competitive threats you identified
- Overstates return potential without realistic comparables
- Ignores stage misalignment or missing key milestones

Then update your stance. You may:
- STAND FIRM: Keep your position if confident, especially if valuation concerns remain unaddressed
...
Be conservative. Highlight valuation risks and market uncertainty. Challenge arguments that rely on optimistic projections without evidence.
"""
...
class ValuationDebateAgent:
    """Wraps ValuationAgent with adversarial debate capability."""

    agent_name: str = "Valuation Agent"

    def __init__(self, risk_tolerance: str = "risk_neutral") -> None:
        """Initialize with a base ValuationAgent instance.

        Args:
            risk_tolerance: Risk tolerance setting for system prompt selection.
        """
        self.risk_tolerance = risk_tolerance
        self._base_agent = ValuationAgent(risk_tolerance)
...
Return JSON in the specified format."""
        ...
        try:
            ...
            logger.warning(f"ValuationDebateAgent: Failed to parse debate response: {e}")
    ...
    @property
    def base_agent(self) -> ValuationAgent:
        """Access the underlying ValuationAgent for Phase 1 analysis."""
